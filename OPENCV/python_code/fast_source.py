    ...
    for y in range(startSearchRow, endSearchRow):
        for x in range(startSearchCol, endSearchCol):
            ...
            if len(window) % 2 == 1:
                ...
                medianValue = int(
                    (window[len(window)//2]+window[len(window)//2+1])/2)
            dst[y][x] = medianValue
    return dst
...
def test():
    image = cv2.imread(
        '/home/ledi/桌面/clion/learn_orbslam/cpp_project_learn/OPENCV/python_code/kaze.png')
    ...
    cv2.imwrite( 'fast_source.png',image)
    # cv2.waitKey(0)
