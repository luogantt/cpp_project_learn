...
string second_file= "/home/ledi/桌面/clion/makfile_learn/slambook2/ch7/orb_self/2.png";
...
void ComputeORB (const cv::Mat &img, vector<cv::KeyPoint> &keypoints, vector<DescType> &descriptors) {
    ...
    for (auto &kp : keypoints) {    //剔除边缘关键点
        ...
        //计算局部区域内所有像素点
        //以关键点为圆心，patch为特征点领域，m10、m01为领域像素灰度质心
        float m01 = 0, m10 = 0;
        ...
        float m_sqrt = sqrt(m01 * m01 + m10 * m10) + 1e-18;//avoid divided by zero;
        ...
        for (int i = 0; i < 8; i++) {
            ...
            for (int k = 0; k < 32; k++) {
                ...
                cv::Point2f qq = cv::Point2f(cos_theta * q.x - sin_theta * q.y,
                                             sin_theta * q.y + cos_theta * q.y) + kp.pt;
    ...
    //每个描述子DescType均为32*8=256位二进制
    //desc1、desc2为32*8的二维向量
    //desc.size()为图像的描述子的数量
    //每个特征点x(t)与所有x(t+1)测量描述子的距离
    //比较排序，取最近的作为匹配点，记录ℹi匹配的i2
    void BfMatch(
